...
1. Запрашиваем все комментарии проекта через `GET /v2/entities/project/{id}/comments`.
2. Фильтруем — оставляем те, что содержат тег `#WeeklyStatus` (регистронезависимо).
3. Берём самый свежий такой комментарий как «последний статус».
4. Если комментария нет ИЛИ он старше `STATUS_FRESHNESS_DAYS` (по умолчанию 6 дней) → `is_stale = True`, бизнес-статус принудительно = `UNKNOWN`.
5. Иначе — берём `project.entity_status` и маппим через `TRACKER_STATUS_MAP`.

### 2.3. Маппинг статусов Tracker → бизнес-статус

Таблица из `app/status/mapping.py`:

| `entity_status` (API Tracker) | UI Tracker          | `BusinessStatus` | Эмодзи |
|-------------------------------|---------------------|------------------|--------|
| `according_to_plan`           | По плану            | `ON_TRACK`       | 🟢     |
| `at_risk`                     | Есть риски          | `AT_RISK`        | 🟡     |
| `blocked`                     | Заблокирован        | `BLOCKED`        | 🔴     |
| любой другой (`draft`, `in_progress`, `launched`, `paused`, `null`) | — | `UNKNOWN` | ⚪ |

Даже если `entity_status` распознан, но комментарий просрочен, бот показывает `UNKNOWN`: без свежего `#WeeklyStatus` нельзя утверждать, что статус актуален.
...
- `render_risk` → только `AT_RISK` (блокированные НЕ включаются).
- `render_blocked` → только `BLOCKED`.
- `render_on_track` → только `ON_TRACK`.

Пустые списки рендерятся отдельным сообщением (`Проектов с рисками нет.` и т.п.).

### 2.7. Отличие «устарело» от «не заполнено»
...
### 2.8. Двухшаговый диалог

Если для команды уровня (domain/subdomain/team) найдено несколько портфелей, бот сначала присылает нумерованный список; следующим числовым сообщением пользователь выбирает нужный. При одном совпадении шаг пропускается. Состояние хранится в `CommandRouter._pending: dict[chat_id, _PendingSelection]`.
